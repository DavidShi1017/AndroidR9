...
public class AESUtils {
	...
	public static final String MASTERPASSWORD = "NMBS";
	...
	public static String decrypt( String encrypted) throws Exception {
	    byte[] seedByte = MASTERPASSWORD.getBytes();
	    System.arraycopy(seedByte, 0, key, 0, ((seedByte.length < 16) ? seedByte.length : 16));
	    String base64 = new String(Base64.decode(encrypted, 0));
	    byte[] rawKey = getRawKey(seedByte);
	    byte[] enc = toByte(base64);
	...
	public static byte[] encryptPdfOrBarcode(String pwd,InputStream decryptIs) {
		try {
			ByteArrayOutputStream swapStream = new ByteArrayOutputStream(); 
			byte[] buff = new byte[100];
			int rc = 0; 
			while ((rc = decryptIs.read(buff, 0, 100)) > 0) { 
			swapStream.write(buff, 0, rc); 
			} 
			byte[] in_b = swapStream.toByteArray(); 
			return encryptBytes(pwd,in_b);
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		}
	...
	public static byte[] decryptPdfOrBarcode(String pwd,InputStream encrypttIs) {
		try {
			ByteArrayOutputStream swapStream = new ByteArrayOutputStream(); 
			byte[] buff = new byte[100];
			int rc = 0; 
			while ((rc = encrypttIs.read(buff, 0, 100)) > 0) { 
			swapStream.write(buff, 0, rc); 
			} 
			byte[] in_b = swapStream.toByteArray(); 
			return decryptBytes(pwd,in_b);
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		}
