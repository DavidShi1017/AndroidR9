...
import com.cfl.adapter.AlertDeleteConnectionAdapter;
import com.cfl.application.NMBSApplication;
import com.cfl.async.DeleteAllSubScriptionAsyncTask;
import com.cfl.async.DeleteSubScriptionAsyncTask;
 ...
 */
public class AlertDeleteActivity extends BaseActivity {
	private final static String TAG = AlertDeleteActivity.class.getSimpleName();
	private IPushService pushService;
	private AlertDeleteConnectionAdapter alertConnectionAdapter;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		bindView();
		//initData();
		GoogleAnalyticsUtil.getInstance().sendScreen(AlertDeleteActivity.this, TrackerConstant.ALERT_Delete);
	}
	...
	private void initData(){
		...
		if(isDnr){
			...
			subscriptionList = pushService.readAllSubscriptionsNoDnr();
		}
		...
		alertConnectionAdapter = new AlertDeleteConnectionAdapter(AlertDeleteActivity.this,subscriptionList);
		for(int i=0;i<this.subscriptionList.size();i++){
			alertConnectionAdapter.getAlertConnectionView(i, this.dnrReferenceLayout);
		}
	}

	public static Intent createIntent(Context context, boolean isDnr){
		//Log.d(TAG, "isDnr...." + isDnr);
		Intent intent = new Intent(context, AlertDeleteActivity.class);
		...
		runOnUiThread(new Runnable() {
			public void run() {
				if (progressDialog == null) {
					...
					progressDialog = ProgressDialog.show(AlertDeleteActivity.this, null,
	...
	class ServiceDeleteSubscriptionReceiver extends BroadcastReceiver {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if(isDeleteAll){
				...
				switch (resultNumber){
					case 0:
						dialogError = new DialogAlertError(AlertDeleteActivity.this, getResources().getString(R.string.alert_delete_failure_title),
